...
uses
  ...
  Vcl.Menus, Vcl.Mask, Vcl.ComCtrls, UnitEquacao;
...
begin
  ...
  Begin}
    Confirmasair2 := MessageDlg('Confirma sair da Calculadora? ', TMsgDlgType.mtConfirmation, mbYesNo, 0);
...
begin
   Showmessage('Calculadora criada');
end;
...
begin
    ...
    if ENumeros = True then
    begin
        if (EditNumero1.text = '0') and (StrToCurr(EditNumero2.text) <= -1) then
        Begin
           EditResultado.text := 'Não existe essa potência';
           EditResultado.Font.Color := ClRed;
        End
        else
        Begin
...
//Ao clicar no Δ
procedure TFormEstudo.ButtonDeltaClick(Sender: TObject);
begin
    if not assigned(FormEquacao) then
    begin
    FormEquacao := TFormEquacao.Create(nil);
    end;
    FormEquacao.ShowModal;
    EditResultado.text := FormEquacao.ResultadoEquacao;
    if EditResultado.text = 'Sem resultado' then
    begin
       EditResultado.font.color := ClRed;
    end
    else
...
Var
  ...
  begin
    ...
    begin
       for Progresso := 0 to 5000 do
         begin
           ProgressBarConversao.Position := Progresso;
         end;
...
procedure TFormEstudo.Button1Click(Sender: TObject);
begin
  Var
    Exibepessoa: Tpessoa;
  ...
  Showmessage('Nome: ' + Pessoa[1].Nome + #13#10 + 'Idade: ' +
    IntToStr(Pessoa[1].Idade));
